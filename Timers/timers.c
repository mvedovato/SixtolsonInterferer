...
volatile uint32_t bufferPWM1 = 20;
volatile uint32_t bufferPWM2 = 18;
volatile uint32_t bufferPWM3 = 23;
/**
...
void pwm1( void )
{
	...
	if( flagPWM1 )
	{
		...
		if( bufferPWM1 )
		{
			...
			TimerStart(EV_PWM1, bufferPWM1, pwm1, DEC);
			if( bufferPWM1 >= 15 && bufferPWM1 <= 25 )	//200 -> 20ms
			{
				#ifdef	DUTYaCTIVEhIGH
					GPIO_Set( PWM1, ON );
				#else
					GPIO_Set( PWM1, OFF );
				#endif
	...
	{
		flagPWM1 = 1;
		TimerStart(EV_PWM1, 200 - bufferPWM1, pwm1, DEC);
		#ifdef	DUTYaCTIVEhIGH
			GPIO_Set( PWM1, OFF );
		#else
			GPIO_Set( PWM1, ON );
...
void pwm2( void )
{
	...
	if( flagPWM2 )
	{
		...
		if( bufferPWM2 )
		{
			...
			TimerStart(EV_PWM2, bufferPWM2, pwm2, DEC);
			if( bufferPWM2 >= 15 && bufferPWM2 <= 25 )	//200 -> 20ms
			{
				#ifdef	DUTYaCTIVEhIGH
					GPIO_Set( PWM2, ON );
				#else
	...
	{
		flagPWM2 = 1;
		TimerStart(EV_PWM2, VEINTEmILIS - bufferPWM2, pwm2, DEC);
		#ifdef	DUTYaCTIVEhIGH
			GPIO_Set( PWM2, OFF );
		#else
			GPIO_Set( PWM2, ON );
...
void pwm3( void )
{
	...
	if( flagPWM3 )
	{
		...
		if( bufferPWM3 )
		{
			...
			TimerStart(EV_PWM3, bufferPWM3, pwm3, DEC);
			if( bufferPWM3 >= 15 && bufferPWM3 <= 25 )	//200 -> 20ms
			{
				#ifdef	DUTYaCTIVEhIGH
					GPIO_Set( PWM3, ON );
				#else
	...
	{
		flagPWM3 = 1;
		TimerStart(EV_PWM3, VEINTEmILIS - bufferPWM3, pwm3, DEC);
		#ifdef	DUTYaCTIVEhIGH
			GPIO_Set( PWM3, OFF );
		#else
			GPIO_Set( PWM3, ON );
