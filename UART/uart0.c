...
{
	strcpy( (char*)ipServer, (char*)tramaRecibida);
	UART0_PushTx('\n');
	UART0_Send( ipServer, strlen((char*)ipServer) );
...
{
	flagPrimerOctetoOK = 0;
	flagSegundoOctetoOK = 0;
	flagTercerOctetoOK = 0;
}

uint8_t CheckFlagsOctetos( void )
{
	if( flagPrimerOctetoOK && flagSegundoOctetoOK && flagTercerOctetoOK )
		return 1;
	else
		return 0;
}
